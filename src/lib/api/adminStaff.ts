...
export async function listAdminStaff(filters: {
  ...
  const response = await apiClient.get("/accounts/admin-staff/", { params });
...
export async function listBackofficeGroups(): Promise<string[]> {
  const response = await apiClient.get("/accounts/admin-staff/backoffice-groups/");
  return response.data?.groups || [];
}

export async function setStaffBackofficeGroups(staffUserId: string, groups: string[]): Promise<AdminStaffUser> {
  const response = await apiClient.post(`/accounts/admin-staff/${staffUserId}/set-backoffice-groups/`, {
    groups,
  });
...
export async function inviteStaff(payload: {
  ...
  const response = await apiClient.post("/accounts/admin-staff/invite/", payload);
  return response.data;
}

export async function removeStaffAccess(staffUserId: string): Promise<{ success: boolean }> {
  const response = await apiClient.post(`/accounts/admin-staff/${staffUserId}/remove-access/`, {});
  return response.data;
}
